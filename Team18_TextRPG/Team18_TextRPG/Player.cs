...
namespace Sparta_Team18_TextRPG
{
    public class Player
    {
        ...
        public void PlayerAttack(List<Monster> monsterList, int chooseNum)
        {
            Monster targetMonster = monsterList[chooseNum];

            InputValidator inputValidator = new InputValidator();   // 플레이어 입력 유효성 검사 클래스

            // 플레이어의 입력이 유효한 몬스터 번호가 아니거나, 0보다 작을 경우 에러 메세지 출력
            if (!inputValidator.CheckSelectUserInput(monsterList, chooseNum))
            {
                return;
            }
